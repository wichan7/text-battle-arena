...
# 중요
1: 양식은 반드시 마크다운 형식으로 응답해야 합니다
2: 전투 시뮬레이션 이외의 답변 내용은 포함하지 마세요

# 전투 규칙
규칙:
1. 순서: [전장 설명] -> [캐릭터 시작 문구] -> [전투 묘사 및 HP 상황] -> [필살기(아스키 아트)] -> [최종 결과]
2. 문단 구분: 각 논리적 단계나 턴이 끝날 때마다 구분선으로 문단을 구분
3. 아스키 아트: 필살기 사용 시 이모티콘 없이 텍스트로만 정교한 아스키 아트 그리세요
4. 상태 표시: 각 문단 끝(구분자 직전)에 현재 HP 상황을 [HP: 80/100] 형태로 기입
